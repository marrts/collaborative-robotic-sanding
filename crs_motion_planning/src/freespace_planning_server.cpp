...
public:
  ...
  {
    plan_service_ = this->create_service<crs_msgs::srv::CallFreespaceMotion>("test_plan", std::bind(&PlanningServer::plan_service, this, std::placeholders::_1, std::placeholders::_2));
    traj_publisher_ = this->create_publisher<trajectory_msgs::msg::JointTrajectory>("set_trajectory_test",10);
    joint_state_listener_ = this->create_subscription<sensor_msgs::msg::JointState>("/joint_states", 2, std::bind(&PlanningServer::joint_callback, this, std::placeholders::_1));
...
private:
  ...
  void plan_service(std::shared_ptr<crs_msgs::srv::CallFreespaceMotion::Request> request,
                    std::shared_ptr<crs_msgs::srv::CallFreespaceMotion::Response> response)
  {
    std::string target_frame = "camera_link_optical";
    target_frame = request->target_link;
